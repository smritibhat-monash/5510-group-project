...
Alcohol is a psychoactive substance with dependent properties that has been widely used in many cultures for centuries. Countries around the world also have different drinking cultures. But the harmful use of alcohol can impose a heavy disease, social and economic burden on society. As one of the major risk factors for the health of the global population, WHO Member States in 2010 discussed and agreed to an international consensus on the Global Strategy to Reduce the Harmful Use of Alcohol. The reproducible report will also use WHO's definitive data on alcohol and health as a dataset to further analyse alcohol consumption and the impact it has on society.
...
type_time <- left_join(Countries, spirit_data, by = c("admin" = "Country"))
  ...
  animate(t_time, duration = 30, fps = 20, width = 1060, height = 480)
```

...
test_country <- anti_join(beer_data, countries, by = c("Country" = "admin"))

# Modify the country name consisting with rnaturalearth package

beer_country <- beer_data %>%
...
beer_country_time <- left_join(countries, beer_country, by = c("admin" = "Country"))

  b_time  <- ggplot() +
  geom_sf(data = countries, fill = NA) +
  geom_sf(data = beer_country_time %>% 
  ...
  animate(b_time, duration = 30, fps = 20, width = 1400, height = 750, renderer = gifski_renderer())
```
...
trend_total <- c_tidy %>%
  ...
  filter(Year == c(1980:1990))
trend_total

  ggplot(trend_total, aes(x = Year,
                          y = Litres,
                          color = factor(Beverage_Types))) +
    geom_line()
```

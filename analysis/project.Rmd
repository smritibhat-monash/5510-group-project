...
map1 <- world_map %>% 
  ...
  labs(title="Youth drinkers percentage all over the world",
       x = "Longitude", 
       y = "Latitude") +
  theme_light() 
ggplotly(map1)
...
map2 <- world_map %>% 
  ...
  scale_x_continuous(breaks = seq(-180, 210, 45), labels = function(x){paste0(x, "°")}) +
  scale_y_continuous(breaks = seq(-60, 100, 30), labels = function(x){paste0(x, "°")}) +
  labs(title="Youth heavy drinkers percentage all over the world",
       x = "Longitude", 
       y = "Latitude") +
  theme_light() 
ggplotly(map2)
...
map_age <- world_map %>% 
  ...
  scale_x_continuous(breaks = seq(-180, 210, 45), labels = function(x){paste0(x, "°")}) +
  scale_y_continuous(breaks = seq(-60, 100, 30), labels = function(x){paste0(x, "°")}) +
  labs(title="Age limits all over the world",
       x = "Longitude", 
       y = "Latitude") +
  theme_light() 
ggplotly(map_age)
...
map_BAC <- world_map %>% 
  ...
  scale_x_continuous(breaks = seq(-180, 210, 45), labels = function(x){paste0(x, "°")}) +
  scale_y_continuous(breaks = seq(-60, 100, 30), labels = function(x){paste0(x, "°")}) +
  labs(title="Youth BAC limits all over the world",
       x = "Longitude", 
       y = "Latitude") +
  theme_light() 
ggplotly(map_BAC)
...
limits_tidy %>% 
  ...
  ggplot(aes(x = drinker_percentage,
             y = country,
             fill = Age_limit)) +
  geom_col() +
...
```{r}
country_list <- youth_drinking %>% group_by(country) %>% distinct(country)
```
...
```{r fig.height = 10}
p1 <- youth_drinking %>% 
  ggplot(aes(x = at_least_once_a_week_mean,
             y = reorder(country, desc(at_least_once_a_week_mean)))) +
  geom_boxplot() +
...
p2 <- youth_drinking %>% 
  ggplot(aes(x = first_drink_earlier_than_13_mean,
             y = reorder(country, desc(first_drink_earlier_than_13_mean)))) +
  geom_boxplot() +
  scale_y_discrete(labels = NULL) +
  labs(y = "") +
  theme_bw()
...
p3 <- youth_drinking %>% 
  ggplot(aes(x = any_alcohol_in_past_12_months_mean,
             y = reorder(country, desc(any_alcohol_in_past_12_months_mean)))) +
  geom_boxplot() +
  scale_y_discrete(labels = NULL) +
  labs(y = "") +
  theme_bw()
...
p4 <- youth_drinking %>% 
  ggplot(aes(x = any_alcohol_in_past_30_days_mean,
             y = reorder(country, desc(any_alcohol_in_past_30_days_mean)))) +
  geom_boxplot() +
  scale_y_discrete(labels = NULL) +
  labs(y = "") +
  theme_bw()

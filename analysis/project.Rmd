...
spirit_data <- c_tidy %>%
  pivot_longer(cols = All_types:Other_alcoholic_beverages,
               ...
               values_to = "Litres") %>%
  filter(!str_detect(Beverage_Types, "All")) %>%
...
type_time <- left_join(countries, spirit_data, by = c("admin" = "Country"))
  ...
  filter(!is.na(Year)), mapping = aes(fill = Beverage_Types)) +
  ...
  animate(t_time, duration = 30, fps = 20)
```

### Changes in the popularity of various alcoholic beverages

```{r}
seychelles_spirits <- c_tidy %>%
  ...
  labs(title = "Alcohol per capita (15+) consumption (in litres of pure alcohol)",
...
**How has the popularity of each type of alcohol changed over the**

**years for different countries and have the local favorites of earlier**

**decades been overtaken by other types of alcohol in terms of consumption?**
...
```{r}
seychelles_gender <- consumption_g %>%
  filter(Country == "Seychelles") %>%
  ...
  labs(title = "Alcohol per capita (15+) consumption (in litres of pure alcohol)",
       y = "Litres of pure alcohol") +
  geom_text(aes(label = Litres), vjust = -0.5, position = position_dodge(0.9))
```

...
limits_tidy <- limits1 %>% 
  ...
  distinct() %>% 
  full_join(limits2) %>% 
  mutate(BAC_limit = factor(BAC_limits, 
...
p1 <- world_map %>% 
  ...
  labs(title="Drinkers percentage all over the world",
       x = "Longitude", 
       y = "Latitude") +
  theme_light() 
ggplotly(p1)
...
youth_drinking1 <- first_drink_earlier_than_13_tidy %>% 
  ...
  mutate(year = factor(year, levels = as.character(c("2005", "2006", "2010", "2014"))))




#%>%
#  summarise(mean_first_drink_earlier_than_13_percentage = mean(first_drink_earlier_than_13_percentage),
#            mean_at_least_once_a_week_percentage = mean(at_least_once_a_week_percentage)) %>%
 
```

```{r}
country_list <- youth_drinking1 %>% group_by(country) %>% distinct(country)
```

```{r fig.width = 14, fig.height = 8}
p <- world_map %>% 
  left_join(youth_drinking1, 
            by = c("region" = "country")) %>% 
  mutate(year = as.integer(year)) %>% 
  ggplot(aes(x = long, 
             y = lat, 
             group = group)) +
  geom_polygon(aes(fill = first_drink_earlier_than_13_percentage), 
               colour = "gray50") +
  scale_x_continuous(breaks = seq(-180, 210, 45), labels = function(x){paste0(x, "°")}) +
  scale_y_continuous(breaks = seq(-60, 100, 30), labels = function(x){paste0(x, "°")}) +
  labs(title="Drinkers percentage all over the world",
...
library(gganimate)
p+transition_time(year)+shadow_mark(exclude_layer = 3)+
  exit_disappear()
```
...
```{r fig.height = 10}
youth_drinking1 %>% 
  ggplot(aes(x = at_least_once_a_week_percentage,
             y = reorder(country, desc(at_least_once_a_week_percentage)))) +
  geom_boxplot() +
  theme_bw()
```

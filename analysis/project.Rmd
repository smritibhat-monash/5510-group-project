...
```{r}
t_each_type <- t_tidy %>% 
  select(-c("Country", "Year", "All_types")) %>% 
  summarise(Beer = sum(Beer, na.rm = TRUE),
            Wine = sum(Wine, na.rm = TRUE),
            Spirits = sum(Spirits, na.rm = TRUE),
            Other_alcoholic_beverages = sum(Other_alcoholic_beverages, na.rm = TRUE)) %>%
  pivot_longer(cols = c(Beer, Wine, Spirits, Other_alcoholic_beverages),
               names_to = "Types",
               values_to = "Litres") 

  t_pie = plot_ly(t_each_type) %>%
  add_pie(labels = t_each_type$Types, values = t_each_type$Litres,hole=0.6)
  t_pie
```

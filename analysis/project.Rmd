...
Alcohol is a psychoactive substance with dependent properties that has been widely consumed in many cultures for centuries. It has been consumed for a wide variety of purposes from celebration to mourning.

Countries around the world also have different drinking cultures.
...
```{r pressure, echo=FALSE, fig.cap="Disaster", out.width = '100%'}
knitr::include_graphics("meme2.png.webp")
```
...
kbl(df_alc) %>%
  ...
  row_spec(10, bold = T)
```
- From the table we can see that the most efficient action area seems to be - **`r paste(df_alc$'Action Area'[1])`**
- While the least efficient area is - **`r paste(df_alc$'Action Area'[10])`**
- **`r paste(df_alc$'Action Area'[2])`** and **`r paste(df_alc$'Action Area'[3])`** is also said to be efficient however the government refuses to take up these courses of action
```{r}
df1 <- check_tbl %>%
...
- There are lots of causes that can affect the number of alcohol-attributed deaths. 
- The positive causes include education and religions.
- The negative causes include poverty and lack of government interventions such as lack of controls on advertisement and the production of the unauthorized harmful alcoholic breverages.
...
- There are many policy action areas that are being undertaken by countries across the world.
- However their efforts seem focused in the wrong direction.
- The countries need to refocus with primary efforts towards the policy action areas of -

```{r}
df_alc <- head(df_alc, 3)

kbl(df_alc$`Action Area`) %>%
  kable_minimal(font_size = 15) %>%
  row_spec(1, bold = T)
```
...
- There are a lot of factors that influence teen drinking, such as religious beliefs, drinking culture, economic depression, national character, price of alcoholic drinks, natural cold and so on.

- The reason that some of the country's restrictions have little to do with the proportion of youth alcohol drinking is that what matters is whether the policy is strictly enforced and how effective it is.
...
- In Europe, teenagers in Northern Europe drink the least.


**Limitations**

1. Only 15-year-old were analyzed.  
...
```{r pressure2, echo=FALSE, fig.cap="Do You?", out.width = '100%'}
knitr::include_graphics("meme1.jpeg")
```

...
# Research Question 1
_Question has been changed from before. Questions 1 and 2 from previous document have been merged._
...
This filters out columns with no data for the regions of Africa, Americas, Asia, Europe, Mediterranean and the Pacific. The data from all countries is then combined by year as well as action area using a full join. Finally all the countries' policy pattern together are bound together using rbind.
...
The columns are renamed for convenience.The variables X2 and 2018 are dropped as we don't need data source or the trend past 2017. Since our final set of policy data dates back to 2016, we need to retain the figures of 2017 to see their effect. The first row is dropped as it is a repeat of the heading. Then countries are filtered out for whom policy data is not present.
```{r}
...
The trend data is converted into a cleaner table by removing redundant rows and appropriate renaming. This is then pivoted into long form where all the variables except country and year are placed into gender variable with values in percentage.
